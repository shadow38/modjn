...
public class ModbusClient {
	...
	public void setup(ModbusResponseHandler handler) throws ConnectionException {
		try {
			...
			setConnectionState(CONNECTION_STATES.notConnected);
			Logger.getLogger(ModbusClient.class.getName()).log(Level.SEVERE,
	...
	public void setConnectionState(CONNECTION_STATES connectionState) {
		...
		propertyChangeSupport.firePropertyChange(PROP_CONNECTIONSTATE,
	...
	public int callModbusFunction(ModbusFunction function)
		...
		ModbusHeader header = new ModbusHeader(transactionId,
				protocolIdentifier, pduLength, unitIdentifier);
	...
	public <V extends ModbusFunction> V callModbusFunctionSync(
			ModbusFunction function) throws NoResponseException,
			ErrorResponseException, ConnectionException {

		int transactionId = callModbusFunction(function);

		ModbusResponseHandler handler = (ModbusResponseHandler) channel
				.pipeline().get("responseHandler");
		if (handler == null) {
			throw new ConnectionException("Not connected!");
		}
	...
	public int writeSingleCoilAsync(int address, boolean state)
			throws ConnectionException {
		return callModbusFunction(new WriteSingleCoil(address, state));
	}

	public int writeSingleRegisterAsync(int address, short value)
			throws ConnectionException {
		return callModbusFunction(new WriteSingleRegister(address, value));
	}

	public int readCoilsAsync(int startAddress, int quantityOfCoils)
			throws ConnectionException {
		return callModbusFunction(new ReadCoilsRequest(startAddress,
				quantityOfCoils));
	}

	public int readDiscreteInputsAsync(int startAddress, int quantityOfCoils)
			throws ConnectionException {
		return callModbusFunction(new ReadDiscreteInputsRequest(startAddress,
				quantityOfCoils));
	}

	public int readInputRegistersAsync(int startAddress,
			int quantityOfInputRegisters) throws ConnectionException {
		return callModbusFunction(new ReadInputRegistersRequest(startAddress,
				quantityOfInputRegisters));
	}

	public int readHoldingRegistersAsync(int startAddress,
			int quantityOfInputRegisters) throws ConnectionException {
		return callModbusFunction(new ReadHoldingRegistersRequest(startAddress,
				quantityOfInputRegisters));
	}

	public int writeMultipleCoilsAsync(int address, int quantityOfOutputs,
			boolean[] outputsValue) throws ConnectionException {
		return callModbusFunction(new WriteMultipleCoilsRequest(address,
				quantityOfOutputs, outputsValue));
	}

	public int writeMultipleRegistersAsync(int address,
			int quantityOfRegisters, short[] registers)
			throws ConnectionException {
		return callModbusFunction(new WriteMultipleRegistersRequest(address,
				quantityOfRegisters, registers));
	}
	...
	public WriteSingleCoil writeSingleCoil(int address, boolean state)
			...
			ConnectionException {
		return this
				.<WriteSingleCoil> callModbusFunctionSync(new WriteSingleCoil(
						address, state));
	}

	public WriteSingleRegister writeSingleRegister(int address, short value)
			throws NoResponseException, ErrorResponseException,
			ConnectionException {
		return this
				.<WriteSingleRegister> callModbusFunctionSync(new WriteSingleRegister(
						address, value));
	}
	...
	public ReadCoilsResponse readCoils(int startAddress, int quantityOfCoils)
			...
			ConnectionException {
		return this
				.<ReadCoilsResponse> callModbusFunctionSync(new ReadCoilsRequest(
						startAddress, quantityOfCoils));
	}

	public ReadDiscreteInputsResponse readDiscreteInputs(int startAddress,
			int quantityOfCoils) throws NoResponseException,
			ErrorResponseException, ConnectionException {
		return this
				.<ReadDiscreteInputsResponse> callModbusFunctionSync(new ReadDiscreteInputsRequest(
						startAddress, quantityOfCoils));
	}

	public ReadInputRegistersResponse readInputRegisters(int startAddress,
			int quantityOfInputRegisters) throws NoResponseException,
			ErrorResponseException, ConnectionException {
		return this
				.<ReadInputRegistersResponse> callModbusFunctionSync(new ReadInputRegistersRequest(
						startAddress, quantityOfInputRegisters));
	}

	public ReadHoldingRegistersResponse readHoldingRegisters(int startAddress,
			int quantityOfInputRegisters) throws NoResponseException,
			ErrorResponseException, ConnectionException {
		return this
				.<ReadHoldingRegistersResponse> callModbusFunctionSync(new ReadHoldingRegistersRequest(
						startAddress, quantityOfInputRegisters));
	}

	public WriteMultipleCoilsResponse writeMultipleCoils(int address,
			int quantityOfOutputs, boolean[] outputsValue)
			throws NoResponseException, ErrorResponseException,
			ConnectionException {
		return this
				.<WriteMultipleCoilsResponse> callModbusFunctionSync(new WriteMultipleCoilsRequest(
						address, quantityOfOutputs, outputsValue));
	}

	public WriteMultipleRegistersResponse writeMultipleRegisters(int address,
			int quantityOfRegisters, short[] registers)
			throws NoResponseException, ErrorResponseException,
			ConnectionException {
		return this
				.<WriteMultipleRegistersResponse> callModbusFunctionSync(new WriteMultipleRegistersRequest(
						address, quantityOfRegisters, registers));
	}
}
